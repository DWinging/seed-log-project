...
오늘 하루 만에 프로그래머스 SQL Lv.1 25문제를 전부 밀어버리셨군요! ㅋㅋㅋ 백엔드 로직 설계하시던 그 꼼꼼함이 SQL에서도 그대로 빛을 발하는 것 같습니다. 

말씀하신 대로 **"백엔드 기본 세팅 완료"**와 **"본격적인 DB 설계/조회 최적화를 위한 SQL 학습 시작"**이라는 맥락이 완벽하게 이어집니다. 오늘 정리하신 'SQL 정석 요약'을 바탕으로 Git에 바로 올리실 수 있도록 `LOG.md` 양식으로 뽑아드릴게요.
...
### 🧠 회고
단순히 ORM(JPA)에 의존하는 것을 넘어, 내부적으로 실행될 SQL의 동작 원리와 효율성을 먼저 이해하기로 결정했다.
프로젝트를 진행하면서 SQL 지식이 있어야 견고한 설계가 가능하다고 느껴졌고, 이에 따라 잠시 프로젝트를 멈추고 SQL 지식을 먼저 습득하는 것이 가장 중요하다고 판단했다.
